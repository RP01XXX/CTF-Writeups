...
Solution
```
So in challenge 1, we decoded a JWT token, inside this was a username ‘cfodera91’. Use a tool like Sherlock to enumerate other locations where the user has this account.
...
```
Filter the results for LDAP. Go to packet 1503 and follow the stream. Here you see the output of querying all users in the domain. If you look, you will find a flag inside a users description field unencrypted.
...
`Deadface reigns supreme as we continue to tear them down oh by the way the zip password is DoYouReallyThinkYouCanFindMySecretsThatEasily`
...
1. South
    ...
    1. Reviewing the image we see that we have a food menu with a unique logo. If you focus on the logo you find its called a stubborn mule. If you google restaurants in the Orlando with the word ‘mule’ or ‘stubborn mule' you will find the restaurant.
        

        
    3. Review their menu to confirm a match.
        
        
2. West
    
    1. The HTLM is edited to remove the companies name. Pretty easy, Google the License name “License TA298” and find [![](https://ibamusic.com/favicon.ico)IBA Music | An Orlando Based Talent Agency - IBA Music](https://ibamusic.com/) which is located at 1215 E Washington St, Orlando, FL 32801.
        
3. North
    
    1. This one is much easier once we have an idea of where our location is. To find this you want to Google Laundry businesses either in the Zipcode or in Thornton Park. This will result in finding the Thornton Park Laundry Facebook page which has the image as its cover picture.East
        
    2. A simple Google search finds Jacob Summerlin.
        
    3. Look into him and find they are responsible for Lake Eola
        

        
4. East
    
    1. Google image search the image to find the family Summerlin.
        
    2. Quick research shows his family sold Lake Eola to Orlando.
...
1. Using TheSource image, we can assume its somewhere on E Washington Street.
...
The Solution
```
The note has several key factors to it
...
This will be a bit of enumeration to discover these flights. The most efficient wy is that once you have the time of the flight and the 2049 or 92, you can look it up at [![](https://www.flightradar24.com/static/favicons/android-icon-96x96.png?t=1728380576)Live Flight Tracker - Real-Time Flight Tracker Map | Flightradar24](https://www.flightradar24.com/) Simply put the country as the US, then Orlando and look at departures and load earlier flights. You can then find the airline flying out to NY. Do the same to find the Berlin flight.

Now to find the location of the Cafe. If you look at the other clues on the sheet you see Deephax and Coffee? This is a reference to the Vacation chat by Deephax in Ghosttown. If you do some research on his photos, you will find deep1 shows a place called Distrikt. This is the coffee spot that Lillith intends to meet deephax at.
...
The Solution
```
1. First run a simple Exiftool on the image to find the creator Wolfpack01. If you Google this account you will find a lot of different accounts.
    IMAGE

1. Now use the image and look really close at the sign that says 4223 RMHC which will lead you to find you are in Colorado, specifically 38°58'13.3"N 104°45'01.2"W.
    
2. Now you need to figure out what Wolfpack01 hid here.
    
3. If you look up Wolfpack01 and Colorado you will find a Geocaching account. Each of his geocaches are assigned a custom code that starts with 'G' which should be a giveaway that you are on the right track.  

IMAGE

4. Now its a bit of a game of exploring his caches he has posted. Eventually you will find MonkeyRocks' Cache. If you Google maps the coordinates you will land right where the image is.
    

IMAGE

1. This can conclude the code is GC41VBN.
...
This is a continuation from last years OSINT challenge Dark Web Dump but that is not a dependency. Users just may notice the same user again.
IMAGE

They will search in the bio to find the mention of “Adorable-Welcome-268”. for their Github. This is a two parter: first, go to the users github. Look through the commit history to find the IP address needed.

IMAGE

Second using Google or Sherlock you can find this is a Reddit profile.
